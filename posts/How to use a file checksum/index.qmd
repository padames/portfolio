...
Then `sha256sum` would also complain that a hash mismatch happened. This can lead to a false positive by signalling possible file tampering when it is really our mistake creating the hash from the received file:
...
If the length of the hash is changed by mistake the error is different. Below we add a one at the end of the 65 character hash:
